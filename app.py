...
# Default categories shown to new users (UI can override per-user)
DEFAULT_CATEGORIES = [
    "Groceries", "Restaurants", "Transport", "Shopping", "Health",
    "Bills", "Entertainment", "Travel", "Other"
]
...
def _ddb():
    # Lazily create clients (cold start friendly)
    global _DDB
    try:
        return _DDB
    except NameError:
...
def _validate_categories(payload):
    ...
    for c in cats:
        ...
        if not c:
            continue
        if len(c) > 50:
            raise ValueError("Category names must be <= 50 chars")
        cleaned.append(c)
    ...
    if len(dedup) > 50:
        raise ValueError("Max 50 categories")
    return dedup

def _categories_key(sub):
    return {"PK": {"S": f"USER#{sub}"}, "SK": {"S": "CATEGORIES"}}

def get_user_categories(sub):
    # Returns (categories, source)
    if not USER_CATEGORIES_TABLE:
        return (DEFAULT_CATEGORIES, "default")
    ddb=_ddb()
    resp=ddb.get_item(TableName=USER_CATEGORIES_TABLE, Key=_categories_key(sub))
    item=resp.get("Item")
    if not item or "categories" not in item:
        return (DEFAULT_CATEGORIES, "default")
    cats_attr=item["categories"]
    cats=[]
    # stored as DynamoDB String Set or List of Strings
    if "SS" in cats_attr:
        cats=list(cats_attr["SS"])
    elif "L" in cats_attr:
        for x in cats_attr["L"]:
            if "S" in x:
                cats.append(x["S"])
...
def put_user_categories(sub, categories):
    if not USER_CATEGORIES_TABLE:
        raise RuntimeError("USER_CATEGORIES_TABLE not configured")
    ddb=_ddb()
    ddb.put_item(
        ...
        Item={
            **_categories_key(sub),
            ...
            "updatedAt": {"N": str(_now_ms())},
        },
    )

def delete_user_categories(sub):
    if not USER_CATEGORIES_TABLE:
        return
    ddb=_ddb()
    ddb.delete_item(TableName=USER_CATEGORIES_TABLE, Key=_categories_key(sub))
...
if path == "/categories":
    sub = user.get("sub")
    if not sub:
        return json_response({"message": "Unauthorized"}, status=401, origin=origin)

    if method == "GET":
        cats, source = get_user_categories(sub)
        return json_response({"categories": cats, "source": source}, status=200, origin=origin)

    if method in ("PUT", "POST"):
        try:
            payload = json.loads(event.get("body") or "{}")
            cats = _validate_categories(payload)
            put_user_categories(sub, cats)
            return json_response({"categories": cats, "source": "custom"}, status=200, origin=origin)
        except Exception as e:
            return json_response({"message": str(e)}, status=400, origin=origin)

    if method == "DELETE":
        delete_user_categories(sub)
        cats, source = get_user_categories(sub)
        return json_response({"categories": cats, "source": source}, status=200, origin=origin)

    return json_response({"message": "Method not allowed"}, status=405, origin=origin)
